...
class TaskGroup(abc.TaskGroup):
    ...
    def _spawn(
        ...
        _task_states[task] = TaskState(
            parent_id=id(current_task()), name=name, cancel_scope=self.cancel_scope
        )
...
class WorkerThread(Thread):
    ...
    def __init__(
        ...
        self.queue: Queue[Union[Tuple[Callable, tuple, asyncio.Future], None]] = Queue(
            2
        )
...
class TCPSocketListener(abc.SocketListener):
    ...
    async def accept(self) -> abc.SocketStream:
        ...
        return SocketStream(
            cast(asyncio.Transport, transport), cast(StreamProtocol, protocol)
        )
